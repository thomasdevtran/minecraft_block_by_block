...
- Build progress, skin images, preferences and privacy choices use local storage. The privacy page can clear build data while preserving privacy and appearance preferences.
- Only the approximate **opted-in visitor total** is measured. No page-view or daily event tracking; no Vercel Web Analytics script. Counting processes IP/browser data with an HMAC key, then writes a contribution to an Upstash HyperLogLog. It is not a verified count of people. GPC/DNT disables it.
- The footer offers equal allow/decline choices; the privacy page lets visitors withdraw. The server requires the consent header and same-origin request signals. These filters are not proof of a human visitor and do not prevent determined counter manipulation.
...
The counter fails closed in deployed environments when configuration is missing: POST does not count and GET returns 503, rather than publishing a temporary in-memory total. Local development alone can use an in-memory count. Preview deployments do not write to the counter. The new key `bbb:hll:consenting:v1` intentionally does not mix legacy automatic counts with consented counts.

Keep the secret stable for comparable totals. Changing the key changes visitor identities and can overcount against an existing HyperLogLog; it does **not** clear old counts. If rotating a compromised secret, also begin a documented new counting period/key. Review and remove legacy `bbb:hll:all`, `bbb:hll:d:*`, `bbb:views`, and `bbb:hist` records under an approved retention plan; this review did not connect to or delete a production database.

The optional `.github/workflows/record-visitors.yml` snapshots the aggregate in `docs/consenting-visitors.csv`. Set `SITE_URL` to the deployed HTTPS origin to activate it. This is an operational record, not independent proof of audience size. No analytics dashboard is required.
